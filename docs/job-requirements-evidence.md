# Compétences et preuves

| Compétence | Preuve |
|---|---|
| Windows 11 | [Day 07](day07-windows-endpoint-sysinternals.md) |
| TCP/IP et Wireshark | [Day 02](day02-tcpip-foundations.md) |
| Pare-feu, NAT et segmentation | [Day 03](day03-pfsense-segmentation.md) |
| Active Directory et DNS | [Day 04](day04-active-directory-foundation.md) |
| GPO, SMB et permissions | [Day 05](day05-ad-gpo-file-services.md) |
| PowerShell | [Day 06](day06-powershell-automation.md) et [scripts](../scripts/) |
| Support TI et Jira | [Day 07](day07-windows-endpoint-sysinternals.md) et [Day 11](day11-enterprise-operations.md) |
| Entra ID, Intune et Autopilot | [Day 08](day08-entra-intune-autopilot.md) |
| Linux et VPN | [Day 09](day09-fortinet-linux-vpn.md) |
| Wazuh | [Day 10](day10-wazuh-sysmon-incident-response.md) |
| Réponse aux incidents | [IR-001](../operations/incidents/IR-001-Suspicious-Authentication.md) |
| Gestion des changements | [CHG-001](../operations/changes/CHG-001-New-Firewall-Rule.md) |
| ITGC | [Mini revue ITGC](../operations/audit/ITGC-mini-review.md) |
| Cisco, VLAN, HSRP et NAT | [Travaux réseau](../academic-work/README.md) |
| Services Linux | [Travaux Linux](../academic-work/README.md) |
| Cybersécurité | [Travaux cybersécurité](../academic-work/README.md) |
| SNMP | [TP SNMP et MIB](../academic-work/monitoring/420-5F4-tp01-snmp-mib-monitoring.pdf) |
| Python et MariaDB | [Travail Python/MariaDB](../academic-work/programming-databases/420-4H4-travail02-python-mariadb.pdf) |

## À noter

- Day 08 : étude seulement, sans tenant Intune complet.
- Day 10 : serveur et dashboard Wazuh fonctionnels; agent Windows non terminé.
